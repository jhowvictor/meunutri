...
import { Sparkles, ArrowRight, Camera, Loader2, Check } from "lucide-react";
    ...
    <div className="min-h-screen bg-background flex flex-col px-5 py-8 max-w-md mx-auto">
      ...
      <div className="flex-1 space-y-5">
        {step === 0 && (
          <div className="space-y-5 animate-[fadeIn_0.4s]">
            <div>
              ...
              <h1 className="text-3xl font-extrabold">Vamos te conhecer melhor</h1>
              <p className="text-sm text-muted-foreground mt-1.5">Comece com seus dados básicos</p>
            </div>
            <div className="flex flex-col items-center gap-3 py-4">
              ...
              <span className="text-xs text-muted-foreground">Adicione sua foto (opcional)</span>
            </div>
            <Input placeholder="Seu nome completo" value={data.full_name} onChange={(e) => setData({ ...data, full_name: e.target.value })} />
        ...
        {step === 1 && (
          <div className="space-y-5 animate-[fadeIn_0.4s]">
            <div>
              <h1 className="text-3xl font-extrabold">Qual é seu <span className="text-primary">objetivo</span>?</h1>
              ...
              {goals.map((g) => (
                <button key={g} onClick={() => setData({ ...data, main_goal: g })}
                  className={`w-full text-left p-4 rounded-2xl border transition ${
                    data.main_goal === g ? "bg-primary/15 border-primary text-foreground neon-glow-sm" : "bg-white/5 border-white/10 hover:border-white/30"
                  }`}>
        ...
        {step === 2 && (
          <div className="space-y-5 animate-[fadeIn_0.4s]">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1.5">Selecione o que mais combina</p>
            </div>
            <div>
        ...
        {step === 3 && (
          <div className="space-y-5 animate-[fadeIn_0.4s]">
            <div>
              <h1 className="text-3xl font-extrabold">Nível de <span className="text-primary">atividade</span></h1>
              <p className="text-sm text-muted-foreground mt-1.5">Como é sua rotina física?</p>
            </div>
            <div className="space-y-2">
              {activities.map((a) => (
                <button key={a} onClick={() => setData({ ...data, activity_level: a })}
                  className={`w-full text-left p-4 rounded-2xl border transition ${
                    data.activity_level === a ? "bg-primary/15 border-primary neon-glow-sm" : "bg-white/5 border-white/10 hover:border-white/30"
                  }`}>
                  <div className="flex items-center justify-between">
                    <span className="font-medium">{a}</span>
                    {data.activity_level === a && <Check className="h-4 w-4 text-primary" />}
                  </div>
                </button>
              ))}
            </div>
        ...
        {step === 4 && (
          <div className="space-y-6 animate-[fadeIn_0.4s] text-center pt-8">
            ...
            <div>
              <h1 className="text-3xl font-extrabold">Tudo pronto, {data.full_name.split(" ")[0] || "campeão"}! 🎉</h1>
      ...
      <div className="pt-6 flex gap-2">
        ...
        {step < total - 1 ? (
          <Button onClick={() => setStep(step + 1)} className="flex-1 rounded-full bg-primary hover:bg-primary/90 neon-glow-sm font-semibold">
